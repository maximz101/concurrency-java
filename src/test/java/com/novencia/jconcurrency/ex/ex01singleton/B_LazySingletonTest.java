...
public class B_LazySingletonTest {

    @BeforeEach
    void beforeEach() {
        B_LazySingleton.instance = null;
    }

    @RepeatedTest(1000)
    void should_have_single_instance() {
        var s = Collections.synchronizedSet(new HashSet<B_LazySingleton>());
        Runnable runnable = () -> {
            B_LazySingleton instance = B_LazySingleton.getInstance();
            s.add(instance);
        };
