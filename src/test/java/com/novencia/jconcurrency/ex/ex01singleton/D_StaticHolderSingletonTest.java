...
class D_StaticHolderSingletonTest {


    @RepeatedTest(1000)
    void should_have_single_instance() {
        var s = Collections.synchronizedSet(new HashSet<D_StaticHolderSingleton>());
        Runnable runnable = () -> {
            D_StaticHolderSingleton instance = D_StaticHolderSingleton.getInstance();
            s.add(instance);
        };
